...
Number::operator unsigned long() {//преобразование к типу unsigned long
	try { 
		int limit[] = { 4,2,9,4,9,6,7,2,9,5 };//это число-предел unsigned long
		int exit = 0;
         if (this->n > 10) throw overflow_error("Overflow type unsigned long!");//предел unsigned long-10 знаков
		 if (this->n == 10) {//если в числе 10 знаков
		 ...
		 if (exit == 1) throw overflow_error("Overflow type unsigned long!");//если число больше предела,то выбрасываем ошибку
...
ostream& operator<< (ostream &out, Number &numOne) {//вывод в консоль
		out << numOne.toString();
		return out;
}

istream& operator>> (istream &in, Number &numOne) {//ввод из консоли
		in >> numOne.n;
		numOne.setArray();
		for (int i = 0; i < numOne.n; i++) {
			in >> numOne.a[i];
...
ofstream& operator<< (ofstream& os, Number &numOne) {//вывод в текстовый файл
	try {
		if (!(os.is_open())) throw overflow_error("File don't open!");
		os << " ";
		for (int i = 0; i < numOne.n; i++) {
			os << numOne.a[i];
			os << " ";
		}
		os << '/';//признак того,что число записано
		os << endl;
		return os;
	}
	catch (overflow_error &error) {
...
ifstream& operator>> (ifstream& is, Number &numOne) {//ввод из текстового файла
	try {
		if (!(is.is_open())) throw overflow_error("File don't open!");
		char ar[1000];
		int q = 0;
		is >> ar[q];
		q++;
		while (ar[q-1]!='/') {//пока не конец числа
			is >> ar[q];
			q++;
		}
		int len = q - 1;
...
void Number::writetobin(fstream& os) {//запись в двоичный файл
	try {
		if (!(os.is_open())) throw overflow_error("File don't open!");
		char len = this->n + '0';
		os.put(len);
		char *num = new char[this->n];
		for (int i = 0; i < this->n; i++) {
			num[i] = '0' + this->a[i];
			os.put(num[i]);
		}
...
void Number::readfrombin(fstream& os) {//чтение из двоичного файла
	try {
		if (!(os.is_open())) throw overflow_error("File don't open!");
		char len;
		os.get(len);
